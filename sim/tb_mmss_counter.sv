...
module tb_mmss_counter;
  ...
  mmss_counter #(.CLK_HZ(4)) dut(.clk(clk_50mhz), .rst_p(rst_p), .minute_tens(mt),
    .minute_ones(mo), .second_tens(st), .second_ones(so));
  sevenseg_decode decoder(.digit(decode_digit), .segments(decoded_segments));

  task advance(input integer seconds); begin repeat (seconds * 4) @(posedge clk_50mhz); #1; end endtask
  task check_time(input integer emt, input integer emo, input integer est, input integer eso);
    begin
      if (mt !== emt || mo !== emo || st !== est || so !== eso) $fatal(1, "time=%0d%0d:%0d%0d", mt, mo, st, so);
      checks++;
    end
  endtask
